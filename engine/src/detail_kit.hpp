...
// Upright cylinder (optionally capped) around c from z0 to z0 + h: smooth sides.
inline void cylinder(Builder& g, V2 c, float z0, float r, float h, int sides, M side, M top, bool cap = true) {
    g.tube({at(c, z0), at(c, z0 + h)}, r, sides, side);
    if (!cap) return;
    std::vector<V2> ring;
...
inline void water_tank(Builder& g, V2 c, float z, float r, float h, Rng& rng) {
    ...
    cylinder(g, c, z + stand, r, h, 28, M::Tank, M::Tank, false);
    cone(g, c, z + stand + h, r * 1.02f, 0.22f * r, 28, M::Tank);
    // Hoops.
    for (float t : {0.3f, 0.7f}) cylinder(g, c, z + stand + h * t, r + 0.02f, 0.05f, 28, M::Metal, M::Metal, false);
    ...
    g.beam(at(c - d * r * 0.5f, z + stand), at(c - d * (r + 0.6f), z + 0.05f), 0.06f, M::Metal, 10);
}
...
inline void wall_ac(Builder& g, V2 base, V2 n, float z, float w = 0.8f) {
    ...
    g.beam(at(base + n * (d + 0.05f) + t * (w * 0.15f), z + h * 0.5f),
           at(base + n * (d + 0.075f) + t * (w * 0.15f), z + h * 0.5f), h * 0.75f, M::Metal, 20);
...
inline void roof_ac(Builder& g, V2 c, float z, float yaw) {
    ...
    g.beam(at(c, z + 0.9f), at(c, z + 0.95f), 0.7f, M::Metal, 24);
    g.beam(at(c + ax * 0.55f, z + 0.2f), at(c + ax * 1.2f, z + 0.2f), 0.08f, M::Metal, 10);
}
...
inline void dish(Builder& g, V2 c, float z, float r, float yaw) {
    ...
    g.beam(at(c, z), at(c, z + mast), 0.06f, M::Metal, 10);
...
inline void vent(Builder& g, V2 c, float z, float h = 0.8f) {
    cylinder(g, c, z, 0.12f, h, 12, M::Metal, M::Metal);
    cylinder(g, c, z + h + 0.08f, 0.22f, 0.06f, 16, M::Metal, M::Metal);
}
...
inline void wall_stair(Builder& g, const Face& f, float a0, float dir, float z_top) {
    ...
    for (float o : {0.08f, width - 0.02f}) {
        ...
        // Handrail on posts.
        if (o > 0.5f) {
            g.beam(at(f.point(a0, o), 0.95f), at(f.point(a_end, o), z_top + 0.95f), 0.04f, M::Metal, 10);
...
inline void lantern_string(Builder& g, Vec3 a, Vec3 b, int count, Rgb colour) {
    ...
    for (int k = 0; k <= 24; ++k) {
        ...
        wire.push_back(a + (b - a) * t - Vec3{0.0f, 0.0f, sag * 4.0f * t * (1.0f - t)});
    }
    g.tube(wire, 0.008f, 6, M::Metal);
    for (int i = 1; i <= count + 1; ++i) {
        ...
        if (i <= count) {
            ...
            cylinder(g, c, p.z - 0.5f, 0.14f, 0.38f, 14, M::Lantern, M::Lantern);
...
inline void tv_antenna(Builder& g, V2 c, float z, float h, float yaw) {
    g.beam(at(c, z), at(c, z + h), 0.04f, M::Metal, 8);
